...
const AuthPage = () => {
    ...
    try {
      if (mode === "signup") {
        if (!fullName.trim() || !hospital.trim()) {
          toast.error("Исм-шарифи ва касалхона номини киритинг");
          setBusy(false);
          return;
        }
        ...
        toast.success("Аккаунт яратилди");
      } else {
  ...
  const google = async () => {
    ...
    if (result.error) {
      toast.error("Google орқали кириб бўлмади");
      setBusy(false);
    }
  };
        ...
        <div className="w-full max-w-md">
          ...
          <div className="rounded-3xl border border-border bg-card p-7 shadow-md">
            <h1 className="text-2xl font-semibold text-foreground">
              {mode === "signin" ? "Шифокор аккаунтига кириш" : "Янги шифокор аккаунти"}
            </h1>
            <p className="mt-1 text-sm text-muted-foreground">
              {mode === "signin" ? "Email ва паролингиз билан киринг" : "Қисқа маълумот тўлдиринг"}
            </p>
            ...
            <form onSubmit={submit} className="space-y-3">
              {mode === "signup" && (
                <>
                  <div>
                    <Label htmlFor="fn">Исм-шарифи</Label>
                    <Input id="fn" value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Алиев Аброр" required className="rounded-xl mt-1" />
                  </div>
                  <div>
                    <Label htmlFor="h">Касалхона / клиника</Label>
                    <Input id="h" value={hospital} onChange={(e) => setHospital(e.target.value)} placeholder="1-сон шаҳар касалхонаси" required className="rounded-xl mt-1" />
                  </div>
                </>
              )}
              <div>
              ...
              <Button type="submit" disabled={busy} size="lg" className="w-full rounded-xl shadow-md" style={{ background: "var(--gradient-primary)" }}>
                {busy ? <Loader2 className="h-5 w-5 animate-spin" /> : <><Mail className="mr-2 h-5 w-5" /> {mode === "signin" ? "Кириш" : "Рўйхатдан ўтиш"}</>}
              </Button>
            </form>
            ...
            <div className="mt-5 text-center text-sm text-muted-foreground">
              {mode === "signin" ? (
                <>Аккаунтингиз йўқми?{" "}
                  <button onClick={() => setMode("signup")} className="font-medium text-primary hover:underline">Рўйхатдан ўтинг</button>
                </>
              ) : (
                <>Аккаунт мавжудми?{" "}
                  <button onClick={() => setMode("signin")} className="font-medium text-primary hover:underline">Киринг</button>
                </>
              )}
            </div>
